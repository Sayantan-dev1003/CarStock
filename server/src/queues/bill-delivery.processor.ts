...
export class BillDeliveryProcessor {
    ...
    async handleDeliverBill(job: Job<{ billId: string }>) {
        ...
        this.logger.log(`Bill ${bill.billNumber} delivery complete. Email: ${emailSent}, WhatsApp: ${whatsappSent}`);

        // Send push notification (NotificationsService)
        try {
            ...
            this.logger.error(`Failed to send new bill push: ${pushError.message}`);
        }
    }

    @Process(JOB_RESEND_BILL)
    async handleResendBill(job: Job<{ billId: string }>) {
        ...
        if (pdfUrl) {
            // PDF already exists, generate a fresh signed URL
            try {
                pdfUrl = await this.uploadService.getSignedDownloadUrl(`carstock/bills/${bill.billNumber}`);
            } catch (error) {
                this.logger.error(`Failed to get signed URL for bill ${bill.billNumber}: ${error.message}`);
            }
        } else {
            // Regenerate
            let pdfBuffer: Buffer | null = null;
        ...
        try {
            ...
            this.logger.error(`Failed to send resent bill push: ${pushError.message}`);
        }
    }
}
