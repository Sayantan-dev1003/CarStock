...
export class ReportsService {
    ...
    async getDailyData(date?: Date): Promise<DailyReportData> {
        ...
        const [
            ...
            // Query 3 — Top products sold today (Grouping)
            this.prisma.billItem.groupBy({
                by: ['productId'],
                _sum: { quantity: true, total: true },
                where: { bill: { createdAt: { gte: startOfDay, lte: endOfDay } } },
        ...
        // Post-process Top Products to get names
        ...
        // Post-process Payment Breakdown
        ...
        paymentBreakdownRaw.forEach((item) => {
            ...
            if (mode === 'upi') paymentBreakdown.upi = item._sum.total || 0;
            if (mode === 'card') paymentBreakdown.card = item._sum.total || 0;
        });

        // Post-process Revenue by Category
    ...
    async getWeeklyData(weekStart?: Date): Promise<WeeklyReportData> {
        const start = weekStart ? dayjs(weekStart) : dayjs().startOf('week').add(1, 'day'); // Monday
        const end = start.add(7, 'day').subtract(1, 'millisecond');
        ...
        const [
            ...
            // Query 4 — Repeat customers (at least one bill this week and at least one bill before this week)
            this.prisma.customer.count({
                where: {
                    bills: {
            ...
            this.prisma.billItem.groupBy({
                by: ['productId'],
                _sum: { quantity: true, total: true },
                where: { bill: { createdAt: { gte: start.toDate(), lte: end.toDate() } } },
                orderBy: { _sum: { total: 'desc' } },
                take: 5,
            }),
            // Query 7
        ...
        const percentageChange = lastWeekRev === 0 ? 100 : ((thisWeekRev - lastWeekRev) / lastWeekRev) * 100;
        ...
        categoryMap.forEach((rev, cat) => {
            if (rev > maxRev) { maxRev = rev; topCategory = cat; }
        });
        ...
        return {
            ...
            percentageChange,
            topCategory,
            topProducts: topProducts.map(p => ({ name: p.name, quantitySold: p.quantitySold, revenue: p.revenue })),
            lowStockItems: lowStockItems.map(i => ({ name: i.name, currentQuantity: i.quantity, reorderLevel: i.reorderLevel })),
    ...
    async getMonthlyData(month?: number, year?: number): Promise<MonthlyReportData> {
        ...
        const percentageChange = lastMonthRev === 0 ? 100 : ((thisMonthRev - lastMonthRev) / lastMonthRev) * 100;
